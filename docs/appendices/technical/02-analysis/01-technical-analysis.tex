...
\subsection{Data}
\begin{itemize}
    \item SQL Server 2022 in Docker, accessed through EF Core. Migrations are kept in git.
...
\label{sec:git}
The repository is on GitHub. The report is also synced with Overleaf through GitHub, and Coolify deploys from GitHub. The course uses GitLab CI, but the ideas are the same, so we use GitHub Actions.

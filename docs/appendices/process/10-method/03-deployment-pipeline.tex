...
\begin{versionhistory}
  ...
  1.1 & 23-09-2026 & Group 3 & The deploy job requires HTTPS and an exact 200 from the webhook \\
\end{versionhistory}
...
\begin{enumerate}
    ...
    \item Only if \texttt{publish} succeeded does the \texttt{deploy} job run. It calls Coolify's deploy webhook and fails unless Coolify answers \texttt{200}.
...
\subsection*{The deploy job only turns green when Coolify accepted the call}
A green \texttt{deploy} job has to mean that Coolify started a deployment. The first version called the webhook with \texttt{curl --fail}, which only treats \texttt{4xx} and \texttt{5xx} as errors. When the webhook URL was stored with \texttt{http://}, Cloudflare in front of the server answered \texttt{301} to redirect to HTTPS. That is not an error to \texttt{curl --fail}, so the job was green while no deployment had started.

The job now calls the webhook with \texttt{--proto '=https'}, so a plain HTTP URL is refused before any request is sent, and it compares the status code with exactly \texttt{200}. Any other answer, including a redirect, fails the job. The response body is printed in the log, so a failure shows what Coolify or the proxy actually answered. The bearer token is also never sent over plain HTTP, even if the secret is set up wrong.
...
CI holds two secrets as repository secrets: the Coolify deploy webhook URL and a Coolify API token, which the webhook requires as a bearer token. The token is created with only the \texttt{deploy} permission. It is worth noting what the pair can and cannot do: it can start a deployment, and it cannot read environment variables. Compromising it does not expose a database password. Pushing to GHCR uses the workflow's built-in token, so no long-lived registry credential exists to leak in the first place.
...
\begin{itemize}
    ...
    \item \textbf{Coolify: the deploy webhook and the environment variables.} The webhook URL goes into the repository secret \texttt{COOLIFY\_DEPLOY\_WEBHOOK} and an API token with only the \texttt{deploy} permission into \texttt{COOLIFY\_API\_TOKEN}. The URL must start with \texttt{https://}; otherwise the deploy job refuses to call it. Every variable that the compose file marks with \texttt{:?} must be set on the resource.
